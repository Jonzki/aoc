...
namespace Aoc18.Problems
{
    ...
    public class Problem11 : Problem<Input11>
    {
        private int GridSerial { get; set; }

        private int?[,] Grid { get; set; }

        private int?[] Grid2 { get; set; }

        protected override object Part1(Input11 input)
        {
            Console.WriteLine($"Grid Serial: {input.GridSerial}");
            this.GridSerial = input.GridSerial;
            ...
            return $"{blockX},{blockY}: {blockPower}";
        }
        ...
        protected override object Part2(Input11 input)
        {
            Console.WriteLine($"Grid Serial: {input.GridSerial}");
            this.GridSerial = input.GridSerial;
            //this.Grid = new int?[300, 300];
            ...
            var g = new GridX(input.GridSerial);

            var gridSize = 300;
            //Print(gridSize, input.GridSerial);
            ...
            for (int i = 1; i <= gridSize; ++i)
            {
                for (int j = 1; j <= gridSize; ++j)
                {
                    ...
                    for (int s = 1; i + s <= gridSize + 1 && j + s <= gridSize + 1; ++s)
                    {
                        ...
                        if (s == 1)
                        {
                            bp = GetCellPower(i, j, input.GridSerial);
                        }
                        else
                        {
                            ...
                            for (var k = 0; k < s - 1; ++k)
                            {
                                ...
                                t = GetCellPower(x, y, input.GridSerial);
                                block += t;
            ...
            return $"{blockX},{blockY},{blockSize}: {blockPower}";
        }
